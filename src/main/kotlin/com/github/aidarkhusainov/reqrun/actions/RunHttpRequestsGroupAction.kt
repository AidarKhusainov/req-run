...
class RunHttpRequestsGroupAction :
    AnAction(),
    DumbAware {
    ...
    override fun actionPerformed(e: AnActionEvent) {
        ...
        ProgressManager.getInstance().run(
            object : Task.Backgroundable(project, "Executing HTTP requests", true) {
                override fun run(indicator: ProgressIndicator) {
                    ...
                    for (index in blocks.indices) {
                        ...
                        if (unresolved.isNotEmpty()) {
                            val authTokenIds =
                                unresolved.mapNotNull { VariableResolver.extractAuthTokenId(it) }.toSet()
                            val authHeaderIds =
                                unresolved.mapNotNull { VariableResolver.extractAuthHeaderId(it) }.toSet()
                            ...
                            unresolvedErrors += 1
                            log.warn(
                                "ReqRun: unresolved variables for request: " +
                                    VariableResolver.formatUnresolved(unresolved),
                            )
                            continue
                        }
                    ...
                    if (missingAuthIds.isNotEmpty()) {
                        ApplicationManager.getApplication().invokeLater {
                            if (project.isDisposed) return@invokeLater
                            val label =
                                if (missingAuthIds.size == 1) {
                                    "Missing auth config: "
                                } else {
                                    "Missing auth configs: "
                                }
